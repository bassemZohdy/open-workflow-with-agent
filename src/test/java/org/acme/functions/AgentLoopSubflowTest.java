...
class AgentLoopSubflowTest {
    ...
    // --- Orchestration-only subflows (mock A2A endpoints, no LLM involved) ---

    @Test
    void parallelAgentFansOutAcrossAllThreeWorkerBranches() {
        given()
                .contentType("application/json")
                .body("{\"prompt\":\"Review the architecture\"}")
                .when()
                .post("/parallel_agent")
                .then()
                .statusCode(201)
                .body("workflowdata.status", equalTo("completed"))
                .body("workflowdata.parallel_summary", notNullValue())
                .body("workflowdata.research_result.target_agent", equalTo("researcher_agent"))
                .body("workflowdata.coder_result.target_agent", equalTo("coder_agent"))
                .body("workflowdata.reviewer_result.target_agent", equalTo("reviewer_agent"));
    }

    @Test
    void chainAgentPassesEachStepOutputToTheNextStep() {
        given()
                .contentType("application/json")
                .body("{\"prompt\":\"Build a feature\"}")
                .when()
                .post("/chain_agent")
                .then()
                .statusCode(201)
                .body("workflowdata.status", equalTo("completed"))
                .body("workflowdata.chain_summary", notNullValue())
                .body("workflowdata.step1_output.target_agent", equalTo("researcher_agent"))
                .body("workflowdata.step2_output.target_agent", equalTo("coder_agent"))
                .body("workflowdata.step3_output.target_agent", equalTo("reviewer_agent"));
    }

    @Test
    void supervisorAgentRoutesByTaskType() {
        given()
                .contentType("application/json")
                .body("{\"prompt\":\"Write the code\",\"task_type\":\"code\"}")
                .when()
                .post("/supervisor_agent")
                .then()
                .statusCode(201)
                .body("workflowdata.status", equalTo("completed"))
                .body("workflowdata.supervisor_status", notNullValue())
                .body("workflowdata.supervisor_output.target_agent", equalTo("coder_agent"));

        given()
                .contentType("application/json")
                .body("{\"prompt\":\"Review it\",\"task_type\":\"review\"}")
                .when()
                .post("/supervisor_agent")
                .then()
                .statusCode(201)
                .body("workflowdata.supervisor_output.target_agent", equalTo("reviewer_agent"));

        given()
                .contentType("application/json")
                .body("{\"prompt\":\"Research it\"}")
                .when()
                .post("/supervisor_agent")
                .then()
                .statusCode(201)
                .body("workflowdata.supervisor_output.target_agent", equalTo("researcher_agent"));
    }
}
