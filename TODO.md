...
### 2. Sub-Flow Reasoning Loops & Guardrails
...
- [x] **Human-in-the-Loop Approval Gate Sub-Flow (`hitl-gate.sw.yaml`)**: Pauses and gates sensitive tool executions pending approval.
...
- [x] **Step 1**: Implement Memory Catalog (`memory-catalog.yaml`) & REST service endpoints (`/functions/memory/*`).
- [x] **Step 2**: Implement Human-in-the-Loop (HITL) Catalog (`hitl-catalog.yaml`) & REST service endpoints (`/functions/hitl/*`).
- [x] **Step 3**: Implement Output Guardrails Catalog (`guardrails-catalog.yaml`) & REST service endpoints (`/functions/guardrails/*`).
- [x] **Step 4**: Implement HITL Approval Gate Sub-Flow (`hitl-gate.sw.yaml`).
- [x] **Step 5**: Update `tool-executor.sw.yaml` & `agent-loop.sw.yaml` to integrate Memory, HITL, and Guardrails in a generic, abstract way.
- [x] **Step 6**: Update test suite (`UtilityResourceTest.java`) and verify clean build.
- [x] **Step 7**: Update documentation & push to GitHub.
